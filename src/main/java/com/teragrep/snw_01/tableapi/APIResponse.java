...
public class APIResponse {
    ...
    public List<JsonObject> getEvents() {
        ...
        InputStream fis = new ByteArrayInputStream(response.getBytes(StandardCharsets.UTF_8));
        JsonReaderFactory jsonReaderFactory = Json.createReaderFactory(new HashMap<>());
        try (JsonReader reader = jsonReaderFactory.createReader(fis, StandardCharsets.UTF_8)) {
            JsonObject jsonObject = reader.readObject();

            if (!jsonObject.isNull("result")) {
                JsonArray jsonArray = jsonObject.getJsonArray("result");
                for (JsonValue jsonValue : jsonArray) {
                    eventList.add(jsonValue.asJsonObject());
                }
            }
        }
